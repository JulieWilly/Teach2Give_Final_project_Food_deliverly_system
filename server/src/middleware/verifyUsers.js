const verifyUserDetails = async (req, res, next) => {
    try{ 
            ...
            if(!custName) return res.status(400).json('Please provide a the customer name.')
            if(!custEmail) return res.status(400).json('Please provide a the customer Email address.')
