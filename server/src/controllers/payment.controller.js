...
export const getAllPayments = async (req, res) => {
 ...
 try{
    ...
    if (getAllPayments !== null) {
        return res.status(200).json({success: true, message: "Payment found successfully.", data:getAllPayments})
    } else{
        return res.status(500).json({success: false, message: 'Payments has not been found.'})
    }
...
export const getSinglePayment = async (req, res) => {
  ...
  try{
    ...
    res.status(500).json({success: true, message: 'Payment details found successfully.', data: getPayment})
...
export const makePayment = async (req, res) => {
    ...
    try{
        ...
        if (makePayment !== null) {
            ...
            return res.status(500).json({success: false, message: 'Payment not made.Something went wrong!!!'})
        }
    } catch(error) {
        return res.status(500).json({success:false, message: error.message})
    }
...
export const deletePayment = async (req, res) => {
  ...
  try{
   ...
   res.status(200).json({ success: true, message: "Payment details deleted successfully."})
  }catch(error) {
    return res.status(500).json({success: false, message: error.message})
  }
};
