...
export const getAllReviews = async (req, res)=> {
    ...
    try {
        ...
        if(getAllReviews !== null) {
            return res.status(200).json({success: true, message: 'Products have been found successfully.', data: getAllReviews})
        } else {
            return res.status(500).json({success: false, message: "Products have not been found."})
        }
...
try{
   ...
   if (!review) {
    ...
    if (getReview !== null) {
        return res.status(200).json({success: true, message: 'Reviews found successfully.', data:getReview})
    } else {
        return res.status(500).json({success: false, message: 'Reviews not found .'})
    }
...
export const createReview = async (req, res)=> {
    ...
    try{
        ...
        if ( createReview !== null) {
            ...
            return res.status(500).json({success: false, message: 'Review not created. Soomething went wrong!!'})
        }
